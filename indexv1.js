...
// دالة للتواصل مع Gemini API (محسنة)
async function askGemini(prompt, context = '') {
    try {
        const fullPrompt = context ? `${context}\n\nالسؤال: ${prompt}` : prompt;
        
        // استخدام موديل 1.5-flash المستقر والسريع
        ...
        const response = await fetch(
            `https://generativelanguage.googleapis.com/v1beta/models/${modelName}:generateContent?key=${GEMINI_API_KEY}`,
            {
                ...
                body: JSON.stringify({
                    ...
                    // إعدادات لتقليل حظر المحتوى (Safety Settings)
                    safetySettings: [
        ...
        if (!response.ok) {
            ...
            return `عذراً، حدث خطأ في الاتصال بالذكاء الاصطناعي (رمز: ${response.status})`;
        }
        ...
        if (data.candidates && data.candidates.length > 0) {
            ...
            console.error('[⚠️] Blocked by safety filters:', data.promptFeedback);
            return "عذراً، لم أتمكن من الإجابة بسبب قيود المحتوى.";
        } else {
            return "عذراً، لم تصل إجابة واضحة من النظام.";
        }
    } catch (error) {
...
client.on('message_create', async message => {
    try {
        ...
        // ----------------------------------------------------
        // أوامر إغلاق وفتح المجموعة (مضافة حديثاً)
        // ----------------------------------------------------

        // أمر إغلاق المجموعة
        if (isGroupMessage && (content === '!غلق' || content === '!إغلاق' || content === '!close')) {
            if (await isAdmin(userId, currentGroupId)) {
                if (await isBotAdmin(currentGroupId)) {
                    try {
                        ...
                        // تعيين إرسال الرسائل للمشرفين فقط
                        await chat.setMessagesAdminsOnly(true);
                        await message.react('🔒');
                        await client.sendMessage(replyTo, `🔒 *تم إغلاق المجموعة!*\nالآن المشرفون فقط يمكنهم الإرسال.${signature}`);
                    } catch (error) {
                        console.error('[❌] Error closing group:', error);
                        await client.sendMessage(replyTo, `⚠️ حدث خطأ أثناء إغلاق المجموعة.${signature}`);
                    }
                } else {
                    await client.sendMessage(replyTo, `⚠️ البوت ليس مشرفاً في هذه المجموعة، لذا لا يمكنه إغلاقها!${signature}`);
                }
            } else {
                await client.sendMessage(replyTo, `⚠️ هذا الأمر للمشرفين فقط!${signature}`);
            }
            return;
        }

        // أمر فتح المجموعة
        if (isGroupMessage && (content === '!فتح' || content === '!open')) {
            if (await isAdmin(userId, currentGroupId)) {
                if (await isBotAdmin(currentGroupId)) {
                    try {
                        const chat = await client.getChatById(currentGroupId);
                        ...
                        await message.react('🔓');
                        await client.sendMessage(replyTo, `🔓 *تم فتح المجموعة!*\nالآن يمكن للجميع الإرسال.${signature}`);
                    } catch (error) {
                        console.error('[❌] Error opening group:', error);
                        await client.sendMessage(replyTo, `⚠️ حدث خطأ أثناء فتح المجموعة.${signature}`);
                    }
                } else {
                    await client.sendMessage(replyTo, `⚠️ البوت ليس مشرفاً في هذه المجموعة، لذا لا يمكنه فتحها!${signature}`);
                }
            } else {
                await client.sendMessage(replyTo, `⚠️ هذا الأمر للمشرفين فقط!${signature}`);
            }
